...
vector<MIDI_Reader> MIDI_Reader::readFolder(string folderPath)
{
    vector<MIDI_Reader> midis;
    struct dirent* entry;

    DIR* dir = opendir(folderPath.c_str());
    if(dir == NULL) return midis;
    while((entry = readdir(dir)) != NULL)
    {
        if(string(entry->d_name).find(".mid") != string::npos)
        {
            cout << entry->d_name << endl;
            MIDI_Reader m(folderPath + "/" + string(entry->d_name));
            midis.push_back(m);
        }
    }
    closedir(dir);
