...
MotionGenerator::MotionGenerator(double speed_factor, const std::array<double, NUM_PANDA_JOINTS> q_goal) : q_goal_(q_goal.data()) {
...
bool MotionGenerator::calculateDesiredValues(double t, Vector7d* delta_q_d) const {
    ...
    std::array<bool, NUM_PANDA_JOINTS> joint_motion_finished{};

    for (Eigen::Index i = 0; i < NUM_PANDA_JOINTS; i++) {
...
franka::JointPositions MotionGenerator::operator()(const franka::RobotState& robot_state,
    ...
    std::array<double, NUM_PANDA_JOINTS> joint_positions{};
    Eigen::VectorXd::Map(&joint_positions[0], NUM_PANDA_JOINTS) = (q_start_ + delta_q_d);
